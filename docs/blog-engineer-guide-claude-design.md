...
ここは2つのコマンドを順に実行するだけです。

💬 **Claude Code で実行**：

1. `/design-login` — claude.ai のデザインシステムアクセスを認可する（初回のみ）。表示された URL / 手順に従って承認する。
2. `/design-sync` — `@ds/ui` を Claude Design に同期する。同期対象のコンポーネント範囲と、同期先のデザインシステム名を聞かれる。

🧑‍💻 **エンジニアが判断すること**：`/design-sync` 実行中に同期先を聞かれたら、**既存のチームのプロジェクトは汚さず「新規作成」を選ぶ**。ここだけが意思決定で、あとはコマンドに任せられます。

`/design-sync` は内部で「書き込みパスの確定 → ランタイム設置 → `.dc.html` 書き込み」を行います（コマンドの中身を覚える必要はありません）。所要時間は小規模なら約10分、大規模リポジトリでは数時間かかることがあります。

> 環境によっては `/design-sync` スキルが露出せず、Claude Code が Claude Design の MCP ツールを直接使う形になることもあります。その場合も「`@ds/ui` を Claude Design に新規プロジェクトとして同期して」と自然言語で頼めば同じ結果になります。
